...
if need_to_BLAST: #making fasta file
for BLAST database
...
i2 = i1 + '-refseqname'
f.write(f '>{i2}\n{i1}\n')
f.close()# BLAST command, making nucleotide database 'ref'
...
for file in content:
    ...
    files.append(file)# parsing not_matched files in folders
for j in files:
    nb = pd.read_csv(f '{p}\\{j}')# not_matched table# print(len(nb[~nb['barcode'].isin(barcodes)]))
...
f.close()
os.system(f 'cd {path}')
print(str(datetime.now()) + ' ' + f '{p}\\all.fasta')# BLAST command, searching not_matched sequences in database ref.output file end with blastout3.txt.e - value 10, alignment initiating word size 6, search on the same strand, finds only the best hit.outfile contains different information about alignment in tsv format.
os.system(f 'blastn -query {path}\\all.fasta -db ref -out {path}\\all_blastout.txt -evalue 10 -word_size 6 -strand plus -max_target_seqs 1 -max_hsps 1 -outfmt \"6 qacc qlen sacc slen length nident evalue qstart qend sstart send\" -num_threads 10')# - num_threads 8# subtracting - unknownseqname in outfile.
...
print(check)# filtering distance between sequences
...
m = f[~pd.isna(f['Confirmed_deletion'])].reset_index()[['barcode', 'Confirmed_deletion', 'UPTAG_seqs', 'UPTAG_notes', 'n', 'count', 'qual']]# print(str(datetime.now()))
