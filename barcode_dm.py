...
for file in content:
    ...
    files.append(file)# parsing not_barcoded files in folders
for j in files:
    nb = pd.read_csv(f '{p}\\{j}')# not_barcoded table# print(len(nb[~nb['barcode'].isin(seqs)]))
...
values = [
    check['qend'].values, -check['qstart'].values,
    check['qend'].values, -check['qstart'].values
]
check['tof'] = np.select(conditions, values,
    default = None)
...
check['ind'] = np.select(conditions, values,
    default = None)
...
full = pd.merge(full, ref, left_on = 'barcode', right_on = 'UPTAG_seqs', how = 'left')# stats
